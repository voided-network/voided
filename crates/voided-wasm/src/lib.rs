...
pub fn generate_map(
    ...
    let js_map: HashMap<String, Vec<String>> =
        map.into_iter().map(|(k, v)| (k.to_string(), v)).collect();

    serde_wasm_bindgen::to_value(&js_map).map_err(|e| JsValue::from_str(&e.to_string()))
}
...
pub fn obfuscate(
    ...
    let core_map: std::collections::BTreeMap<char, Vec<String>> = js_map
        .into_iter()
    ...
    let js_result = ObfuscationResult {
...
/// Deobfuscate text
#[wasm_bindgen]
pub fn deobfuscate(obfuscated_text: &str, map: JsValue) -> Result<String, JsValue> {
    let js_map: HashMap<String, Vec<String>> =
        serde_wasm_bindgen::from_value(map).map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    Ok(voided_core::obfuscation::deobfuscate(
        obfuscated_text,
        &core_map,
    ))
}
...
pub fn analyze_map(map: JsValue) -> Result<JsValue, JsValue> {
    let js_map: HashMap<String, Vec<String>> =
        serde_wasm_bindgen::from_value(map).map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    serde_wasm_bindgen::to_value(&js_analysis).map_err(|e| JsValue::from_str(&e.to_string()))
}
